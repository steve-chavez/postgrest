    ...
    context "POST with Prefer headers" $ do
      context "when Prefer: resolution=merge-duplicates is specified" $ do
        it "does upsert on pk conflict" $
          request methodPost "/programming_languages" [("Prefer", "return=representation"), ("Prefer", "resolution=merge-duplicates")]
            [json| [
      ...
      context "when Prefer: resolution=ignore-duplicates is specified" $ do
        it "ignores records on pk conflict" $ do
          request methodPost "/programming_languages" [("Prefer", "return=representation"), ("Prefer", "resolution=ignore-duplicates")]
            [json|[
              { "name": "PHP", "rank": 9 },
              { "name": "Python", "rank": 10 }
            ]|] `shouldRespondWith` [json|[
          ...
          get "/programming_languages?rank=gte.9" `shouldRespondWith`
